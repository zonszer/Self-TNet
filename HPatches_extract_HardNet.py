...
parser.add_argument("--hpatches_dir", default='/home/dayong/CV/registration/ZJH/hpatches-benchmark/data/hpatches-release')
parser.add_argument("--output_dir", default='HP_descs')
parser.add_argument("--bs", type=int, default=2560)
...
desc_name =  args.model_name
model.eval()
if model.name == 'TeacherNet':
    model = model.student.cuda()
try:
    ps = model.isize
    print('ps set to', ps)
except:
...
for seq_path in pbar:
    ...
    for tp in tps:
        ...
        for batch_idx in range(n_batches):
            ...
            with torch.no_grad():
                out_a = model(data_a)
                if isinstance(out_a, type({ })):
                    out_a =out_a['desc']
                if isinstance(out_a, type((0,1))):
                    out_a =out_a[0]
