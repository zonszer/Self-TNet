...
class AMOS_dataset(data.Dataset):
    ...
    patch_size:int = 96  # or 64 patch_size(int, default 128): Output patch size
    ...
    def __post_init__(self):
        ...
        self.transform2=trans_centercrop
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            # patches2 = [self.transform2(c) for c in patches]
        if self.fliprot:
        ...
        return {'data':torch.stack(patches, 0), #'centercropped_data':torch.stack(patches2, 0),
                'PS_idxs':PSs[0],'set_idxs':t[1][0],'labels':PSs+self.label_offset, 'modality_idx': t[1]}
    ...
    def generate_tuples(self, num_ofruns, bs_seq):
        tuples = []
        all_cams = np.unique(self.cam_idxs.numpy().astype(np.int)).tolist()     # all_cams: the num of subfolders of the dataset
        ...
        if self.new_batches:
            ...
            def wrap_fce(all_cams, patches_mask, patch_sets_shape, cam_idxs, collisions, n_positives, produce_random_tuple):
                def fce(cur_bsize):
                    ...
                    while True:
                        ...
                        if produce_random_tuple:
                            cur_pair = random.sample(list(range(patch_sets_shape)), n_positives)    #general settings
                        else:
                            cur_pair = list(range(patch_sets_shape))[0: n_positives]
                        valid_init = np.sum(patches_mask[cur_sidxs][:, np.array(cur_pair)], 1) >= 2  # only detected ones
                        left_sidxs = cur_sidxs[valid_init]
                        chosen = set()
                        for i,cur_sidx in enumerate( np.random.permutation(left_sidxs) ):       #imgpatchpair
            ...
            # if num_ofruns != 0:
            # p.restart()
            tuples = map(wrap_fce(all_cams.copy(), self.patches_mask, self.patch_sets.shape[1], cam_idxs, self.collisions, self.Npositives, self.random_tuple), bs_seq)
            # p.close(); p.join()# XXX.map(func, iterable, chunksize=None) #iterable，func，list
    ...
    def generate_LAFs_and_patches_from_dir(self, num_ofruns, dir_name, nLAFs):
        ...
        for img in imgs:
            new_patches = map(crop_patch(img, self.patch_size), As, angles)
        ...
        if self.patch_gen in ['new'] and self.use_patchmask:
            ...
            def fce(centers_single, scales_single):
                ...
                return patches_mask
            patches_mask = map(fce, centers_singles, scales_singles) # parallel over imgs
    ...
    def get_collisions(self):
        ...
        # p = pp.ProcessPool(1)
        # p.restart()
        collisions = map(wrap_fce(self.patch_size, self.LAFs), cam_idxs)
        # p.close(); p.join()
    ...
    def get_collisions_(self, cam_idxs, LAFs):
        ...
        # p = pp.ProcessPool(1)
        # p.restart()
        collisions = map(wrap_fce(self.patch_size, LAFs), cam_idxs)
        # p.close(); p.join()
        return list(itertools.chain.from_iterable(collisions))
    ...
    def prepare_dataset(self, Ntracks, overwrite=False):
        if os.path.exists(self.data_path) and not overwrite:
            ...
            # self.data = {'patch_sets': self.data[0], 'LAFs': self.data[1], 'cam_idxs': self.data[2]}
...
def get_imgs(dir_in, grayscale:bool, depth_path=''):
    ...
    img_fnames = sorted([x for x in files if os.path.splitext(x)[1].strip('.') in ['jpg', 'jpeg', 'png', 'ppm', 'bmp', 'tiff']])
