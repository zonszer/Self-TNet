...
def get_test_loaders(names, bsize, model_arch):
    trans=trans_resize64_affine if '64' in model_arch else trans_resize32
    aux = [
        torch.utils.data.DataLoader(
            TestLoader_6Brown(root=os.path.join('Datasets/6Brown'), name=name, transform=trans),
            batch_size=bsize, shuffle=False, **{'num_workers': 7, 'pin_memory': False})
        for name in names
    ]
...
def get_test_loaders_new(names, bsize, model_arch):
    transform=trans_resize64 if model_arch=='MetricsNet' else trans_resize32
    if 'NIR_RGB_ref_testset' in names:
        aux = [
            torch.utils.data.DataLoader(
                TestLoader_6Brown(root=os.path.join('Datasets/NIR_RGB_ref'), name=name, transform=transform),
                batch_size=bsize, shuffle=False, **{'num_workers': 5, 'pin_memory': True})
            for name in names
        ]
    else:
        aux = [
            torch.utils.data.DataLoader(
                TestLoader_6Brown(root=os.path.join('Datasets/6Brown'), name=name, transform=transform),
                batch_size=bsize, shuffle=False, **{'num_workers': 5, 'pin_memory': True})
            for name in names
        ]
    for a in aux: # for fastai2
...
class DS_wrapper:
    ...
    def prepare_epoch(self, gen_tuples=True, good_patches = None):
        # p = pp.ProcessPool(1); p.close(); p.join()
        ...
        if not gen_tuples:
            return
        # ds_idx_seq used when multiple datasets are used to train
        ...
        if self.args.combine in ['epoch']:
            ...
            for iii in range(len(self.datasets)):
                self.ds_idx_seq += [iii] * int(self.n_iters() / len(self.datasets)) #self.n_iters() = 1627
        ...
        self.iters = []
        # p = pp.ProcessPool(1)       #bs_seq: batch_size num sequence in the trian order
        for i,l in enumerate(self.loaders):
            l.generate_tuples( num_ofruns = i, bs_seq=[l.bs for c in self.ds_idx_seq if c == i])  #len(self.loaders) == 2(）-> bs_seq:batchsize seq
            self.iters += \
    ...
    def __getitem__(self, idx):
        ...
        sample = next(self.iters[ds_idx])       #itersepochtuples. sampledataloaderbatch，for
        info['loader'] = 'other'
        ...
        info['patch_data'] = data
        info["model_arch"] = self.args.model_arch
        ...
        if self.args.model_arch == 'TwoSAE' or self.args.model_arch == 'MetricsNet':
            if self.Npositives == 2:
                data0, data1 = data[0::2], data[1::2]
                data = [data0, data1]
                info['patch_data'] = data
                info['modality_idx'] = sample['modality_idx']
            else:
                raise 'when use TwoSAE, Npositives must == 2'
        # elif self.args.model_arch == 'TwoNet':
        #     if self.Npositives == 2:
        #         # leav to dataloader
        #         # data_centercropped = trans_centercrop(data)
        #         info['patch_data'] = sample['centercropped_data'].cuda()
        #         return (data, info['patch_data']), info
        else:
            return data, info
...
def get_train_dataset(args, data_name): # returns dataset wrapper
    ...
    wrapper = None
    if args.model_arch in ['TwoSAE']: #'MetricsNet']:
        random_tuple=False
    else:
        random_tuple = True
    if '64' in args.model_arch and args.ds in ['v4']:
        trans_AMOS = transform_AMOS_64
    else:
        trans_AMOS = trans_AMOS_32
    if '64' in args.model_arch and args.ds in ['lib']:
        trans_lib = trans_resize64_affine
    else:
        trans_lib = trans_resize32_affine
    ...
    if args.ds in ['mix']:
        ...
        DSs += [DS_AMOS('Datasets/AMOS-views/AMOS-views-v3', data_name, args.patch_sets, args.weight_function, trans_AMOS,
                        args.patch_gen, args.cams_in_batch, workers=25)]
        ...
        DSs = [
            DS_Brown('Datasets/6Brown/liberty.pt', trans_lib,  workers=19),
        ]
    elif args.ds in ['nd']:
        DSs = [
            DS_Brown('Datasets/6Brown/notredame.pt', trans_lib,  workers=19),
        ]
    elif args.ds in ['yos']:
        DSs = [
            DS_Brown('Datasets/6Brown/yosemite.pt', trans_lib,  workers=19),
        ]
        ...
        DSs = [
            DS_NIR_RGB('Datasets/NIR_RGB_DS/NIR_RGB_DS_train', data_name, args.patch_sets, args.weight_function, trans_AMOS,
                    args.patch_gen, args.cams_in_batch, workers=19, random_tuple=random_tuple),
            # DS_Brown('Datasets/6Brown/liberty.pt', trans_resize32),
        ]
    elif args.ds in ['NIR_RGB_ref_train']:
        DSs = [
            DS_NIR_RGB('Datasets/NIR_RGB_ref/NIR_RGB_ref_train', data_name, args.patch_sets, args.weight_function, trans_AMOS,
                    args.patch_gen, args.cams_in_batch, workers=19, random_tuple=random_tuple),
            # DS_Brown('Datasets/6Brown/liberty.pt', trans_resize32),
        ]
    elif args.ds in ['NIR_RGB_ref_testset']:
        DSs = [
            DS_NIR_RGB('Datasets/NIR_RGB_ref/NIR_RGB_ref_testset', data_name, args.patch_sets, args.weight_function,
                       trans_AMOS,
                       args.patch_gen, args.cams_in_batch, workers=19, random_tuple=random_tuple),
            # DS_Brown('Datasets/6Brown/liberty.pt', trans_resize32),
        ]
    elif args.ds in ['v3','v4','v5']:
        DSs = [DS_AMOS(f'Datasets/AMOS-views/AMOS-views-{args.ds}', data_name, args.patch_sets, args.weight_function,
                       trans_AMOS, args.patch_gen, args.cams_in_batch, workers=19, random_tuple=random_tuple)]
        # DSs = [DS_AMOS(f'Datasets/AMOS-views/AMOS-views-v4_testset-{args.ds}', data_name, args.patch_sets,
        #            args.weight_function, trans_AMOS, args.patch_gen, args.cams_in_batch, workers=25)]
    elif args.ds in ['v4_scale64']:
        DSs = [DS_AMOS(f'Datasets/AMOS-views/AMOS-views-v4', data_name, args.patch_sets, args.weight_function,
                       trans_MetricsNet, args.patch_gen, args.cams_in_batch, workers=19, random_tuple=random_tuple)]
        # DSs = [DS_AMOS(f'Datasets/AMOS-views/AMOS-views-v4_testset-{args.ds}', data_name, args.patch_sets,
        #            args.weight_function, trans_AMOS, args.patch_gen, args.cams_in_batch, workers=25)]
    elif args.ds in ['NIR_RGB_ref_scale64']:
        DSs = [
            DS_NIR_RGB('Datasets/NIR_RGB_ref/NIR_RGB_ref_train', data_name, args.patch_sets, args.weight_function, trans_MetricsNet,
                    args.patch_gen, args.cams_in_batch, workers=19, random_tuple=random_tuple)
        ]
        ...
        DSs = [
            DS_Brown('Datasets/6Brown/liberty.pt', trans_resize32),
            DS_AMOS('Datasets/AMOS-views/AMOS-views-v4', data_name, args.patch_sets, args.weight_function, trans_AMOS, args.patch_gen, args.cams_in_batch, workers=25),
        ]
